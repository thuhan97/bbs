...
@section('content')
    <div class="select-month">
        <select class="w-22 mr-1 browser-default custom-select" name="month" onchange="document.location.href='/thoi-gian-lam-viec?month='+this.value">
    ...
    <div class="list-work-time">
        <div class="row statistic">
            <form method="get">
                ...
                <button type="button" class="btn btn-danger" onclick="exec_submit(0)">Số buổi đi muộn: {{$late}}</button>
            </form>
            <form method="get">
                <input hidden name="month">
                <input hidden name="type" value="ve_som">
                <button type="button" class="btn btn-primary" onclick="exec_submit(1)">Số buổi về sớm: {{$early}}</button>
            </form>
            <form method="get">
                <input hidden name="month">
        ...
        <table class="table table-bordered">
            ...
            @foreach($list_work_times as $work_time)
                <tr>
                    ...
                    <th>
                        <a class="feedback" data-toggle="modal" data-target="#feedback" data-id="{{$work_time['id']}}">Kêu oan</a>
                    </th>
                </tr>
            @endforeach
            </tbody>
